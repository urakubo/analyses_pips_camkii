...
# Molecular ID versus molecular name
...
# Blurr for intensity profiles and rendering

sigma = 2 # (in lattice unit)

# radial distribution profile

rdf_bins = np.arange(0, 40) # or np.arange(0, 35, 2)
...
_m = [-1, 0, 1]
neighbors26 = np.stack(np.meshgrid(_m, _m, _m), axis=-1).reshape(-1, 3)
